...
###Errors
An error may arise if the required packages ("lubridate", "dplyr", "zoo") are not installed on your machine. To install any of these packages run the following:
```{r, eval=FALSE}
install.packages(c("lubridate", "dplyr", "zoo")
```
Once the packages are installed start from Stage 1 again.
