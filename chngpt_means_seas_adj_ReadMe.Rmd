---
title: "Change point means seasonally adjusted"
author: "Bart Huntley"
date: "Wednesday, May 27, 2015"
output: html_document
---

This function uses the "changepoint" package to create a ggplot object of a time series of cover values showing where breaks in means occur.

The function uses the data frame "...mtsd.csv" as the starting point.
This data frame must be created first.

This script has been altered from the original **chngpt_means.R** to use the cpt.means call to extract change points:

1. from a seasonally adjusted time series for 1 site
2. from the trend extracted from 1 site

This script no longer loops through all sites and was developed only for examining what changes to change point identification happens when looking at different parts of the decomposed time series. From looking only at site 1 of the test data, the cpt.means run on the regular time series object appears to be the best option, as points 1 and 2 above, add to the complexity of the code and affect only marginal changes to the location of the change points.
...
Plotting only data for the first site.

Change point means detection on the standard time series, the seasonally adjusted and the trend.
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Normal series
ggplot()+
        geom_point(data=norm, aes(x=x, y=y), colour="black")+
        geom_line(data=norm, aes(x=x, y=y), colour="black")+
        geom_segment(data=cptdfNorm, aes(x=start, y=y, xend=end, yend=y), size = 1, colour="red")+
...
ggplot()+
        ...
        geom_segment(data=cptdfSea, aes(x=start, y=y, xend=end, yend=y), size = 1, colour="red")+
        coord_cartesian(ylim = c(-10, 80))+
        theme_bw()+
        xlab("")+
        ylab("Vegetation Cover %")+
...
ggplot()+
        ...
        geom_segment(data=cptdfTre, aes(x=start, y=y, xend=end, yend=y), size = 1, colour="red")+
        coord_cartesian(ylim = c(-10, 80))+
        theme_bw()+
        xlab("")+
        ylab("Vegetation Cover %")+
